    ...
    for face in faces:
        for i in range(eyes.shape[0]):
            for j in range(i + 1, eyes.shape[0]):
                ...
                # eyes are arrays of the form [minX, minY, maxX, maxY]
                if (leftEye[0] + leftEye[2]) > (rightEye[0] + rightEye[2]):
                    # leftCenter is > rightCenter
                    rightEye, leftEye = leftEye, rightEye  # swap

                if contains(leftEye, rightEye) or contains(rightEye,leftEye):
                    # they overlap. One eye containing another is due to a double detection; ignore it
                    debugPrint('rejecting double eye')
                    continue

                if leftEye[3] < rightEye[1] or rightEye[3] < leftEye[1]:
                    # top of one is below (>) bottom of the other. One is likely a mouth or something, not an eye.
                ...
                if not (contains(face, leftEye) and contains(face,rightEye)):
                    # face contains the eyes. This is our standard of humanity, so capture the face.
...
def debugImg(arr):
    global showMainImg
    showMainImg = False
    toShow = cv2.resize((arr - arr.min()) * (1.0 / (arr.max() - arr.min())), (0, 0), fx=8, fy=8,
...
def getPupilCenter(gray, getRawProbabilityImage=False):
    ...
    IRIS_RADIUS = gray.shape[
                      0] * .75 / 2  # conservative-large estimate of iris radius TODO: make this a tracked parameter--pass a prior-probability of radius based on last few iris detections. TUNABLE PARAMETER
    # debugImg(gray)
    ...
    centers = np.array([[phiWithHist(cx, cy, gradDX, gradDY, gradXcoords, gradYcoords, IRIS_RADIUS) if isDark[cy][
        cx] else 0 for cx in xrange(minXForPupil, dxn.shape[1])] for cy in xrange(dxn.shape[0])]).astype('float32')
...
def getEyeCorner(gray):
    ...
    centers = np.array(
        [[phiCorner(cx, cy, gradDX, gradDY, gradXcoords, gradYcoords) for cx in range(rangeOfXForCorner)] for cy in
         range(dxn.shape[0])])
    ##    debugImg(centers)
    ...
    prior[startNew[0]:endNew[0]][startNew[1]:endNew[1]] = (1 - weightOnNew) * lastCornerProb[startPrior[0]:endPrior[0]][
...
def phi(cx, cy, gradDX, gradDY, gradXcoords, gradYcoords, IRIS_RADIUS):
    ...
    valid = (lengthsSquared > 0) & (
    lengthsSquared < IRIS_RADIUS ** 2)  # avoid divide by zero, only use nearby gradients.
...
def phiWithHist(cx, cy, gradDX, gradDY, gradXcoords, gradYcoords, IRIS_RADIUS):
    ...
    bins = [(1 + binWidth * index) ** 2 for index in
            xrange(numBins + 1)]  # express bin edges in terms of length squared
...
def phiCorner(cx, cy, gradDX, gradDY, gradXcoords, gradYcoords):
    ...
    lengthsSquared = np.square(vecx) + np.square(vecy)
    valid = (lengthsSquared > 0) & (lengthsSquared < RELEVANT_DIST_FOR_CORNER_GRADIENTS) & (
    vecx > 0.4)  # RIGHT EYE ASSUMPTION
    numBins = 10
    ...
    validBinb = valid & ((bins[secondMaxBin] - slop < stat) & (
    stat < bins[secondMaxBin + 1] + slop))  # use only points near the histogram max

    dotProd = np.multiply(vecx, gradDX) + np.multiply(vecy, gradDY)
    dotProda = np.square(dotProd[validBina])  # dot products squared
    dotProdb = np.square(dotProd[validBinb])  # dot products squared
    dotProda = 1.0 - np.divide(dotProda, lengthsSquared[
        validBina])  # make normalized squared dot products, and take 1-them so 0 gets the highest score
    dotProdb = 1.0 - np.divide(dotProdb, lengthsSquared[
        validBinb])  # make normalized squared dot products, and take 1-them so 0 gets the highest score
    ##    import pdb;pdb.set_trace()
...
haarFaceCascade = cv2.CascadeClassifier("C:\opencv-build\sources\opencv\data\haarcascades\haarcascade_frontalface_alt.xml")
haarEyeCascade = cv2.CascadeClassifier("C:\opencv-build\sources\opencv\data\haarcascades\haarcascade_eye.xml")
...
# directInferenceLeftRight - combines probability images from left and right to hopefully reduce noise in
# estimation of pupil offset
    ...
    if leftEye_rightEye:  # if we found valid eyes in a face
        ...
        if directInferenceLeftRight:
            ...
            if PupilSpacingRunningAvg is None:
                ...
                PupilSpacingRunningAvg = (
                                         1 - weightOnNew) * PupilSpacingRunningAvg + weightOnNew * pupilSpacing
                # vector from pupil 0 to pupil 1
            if allowDebugDisplay:
            ...
            jointPupilProb = multiplyProbImages(pupilCenterEstimates[1], pupilCenterEstimates[0],
                                                positionOfZeroWithinOne[::-1],
                                                0)  # the [::-1] reverse the order, so it's YX instead of the XY that these vectors are in
        ...
        if not useSURFReference:  # this code assumes you have drawn a dark dot on your forehead. Should be drawn between the eyes, about the size of the iris.
            dotSearchBox = np.round(
                centeredBox(leftEye_rightEye[0], leftEye_rightEye[1], xDistBetweenEyes * .2, xDistBetweenEyes * .3,
                            -xDistBetweenEyes * .09)).astype('int')
            ...
            if warm > 8:
                ...
                if virtualpoint == None:  # we haven't set up the reference point yet
                    ...
                    if len(haystackKeypoints) != 0:
                        betweenEyes = (np.array(featureCenterXY(leftEye_rightEye[0])) + np.array(
                            featureCenterXY(leftEye_rightEye[1]))) / 2
                        virtualpoint = ClassyVirtualReferencePoint.ClassyVirtualReferencePoint(haystackKeypoints,
                                                                                               haystackDescriptors, (
                                                                                               betweenEyes[0],
                                                                                               betweenEyes[1]), face,
                                                                                               leftEye_rightEye[0],
                    ...
                    if len(descriptors) != 0:
                        refXY = virtualpoint.getReferencePoint(keypoints, descriptors, face, leftEye_rightEye[0],
                                                               leftEye_rightEye[1], imgToDrawOn)
        ...
        if allowDebugDisplay:
            # this line show the videocapture
            cv2.imshow(WINDOW_NAME, cv2.resize(output, (0, 0), fx=2, fy=2, interpolation=cv2.INTER_NEAREST))
...
def main():
    ...
    # TODO : save videocapture as video
    # Define the codec and create VideoWriter object
    # fourcc = cv2.cv.CV_FOURCC(*'H264')
    out = cv2.VideoWriter('output.avi', cv2.VideoWriter_fourcc(*'XVID'), 25, (200, 200), True)
    try:
